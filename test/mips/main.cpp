...
int main(int argc, char *argv[]){
    ...
    jcpu::jcpu *const mips = jcpu::jcpu::create("mips", "mips32");
    dummy_mem mem(argv[1], *mips);
    mips->set_ext_interface(&mem);
    mips->reset(true);
    mips->reset(false);
    mips->run(mips->RUN_OPTION_NORMAL);
    //mips->run(mips->RUN_OPTION_WATI_GDB);
